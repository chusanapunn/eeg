...
##############################################################################
        ...
        self.precisionDigit = 2   # Patient info digit precision
        self.band_freqs = {
            "D": (0.5, 3),   # Delta
            "T": (3, 8),     # Theta
            "A": (8, 13),    # Alpha
    ...
    def add_segment(self, segment_name, segmentData, segmentTimes, is_extend, interval_datapoint): # , OVERLAPPING
        ...
        # OVERLAPPING = np.round(OVERLAPPING, self.precisionDigit)
        # self.nOVERLAPPING = OVERLAPPING * self.SAMPLING_FREQUENCY

        seg_suffix="_Extend " if (is_extend and (segment_name in ["Q1","Q2","Q3","Q4"])) else "_Intend "
        seg_name = segment_name+seg_suffix # +str(OVERLAPPING)+' %'
        
        self.segments[seg_name] = {
            # 'segment_name':segment_name,
            'segmentData': {ch: np.round(segmentData[ch_id]*1e6,self.precisionDigit) for ch_id, ch in enumerate(self.ch_list)},
            'segment_times': {ch: np.round(segmentTimes[ch_id],self.precisionDigit) for ch_id, ch in enumerate(self.ch_list)},
            'is_extend': is_extend,
            # 'win_num' : win_num,
        ...
        # print("Patient ",self.subject_id," --- RUNTIME : %.2f seconds ---" % (time.time() - self.start_time))
    ...
    def compute_absolute_power(self, segment_name):
        # self.segments[segment_name]['absolute_power'][ch][band] = 1*self.segments.get('segmentData')[ch]  
        for ch in self.ch_list:         
            data = self.segments[segment_name]['segmentData'][ch]

            ##############################################################

            window_width = 2* self.SAMPLING_FREQUENCY
            fw, Pxx = welch(data, fs=self.SAMPLING_FREQUENCY, nperseg = window_width) #, noverlap=self.nOVERLAPPING
            ...
            for band in self.band_freqs:
                ...
                band_power = np.round(band_power,self.precisionDigit)
    ...
    def compute_relative_power(self, segment_name, ch):
        ...
        for band in self.band_freqs: # power of the specified band
            ...
            self.segments[segment_name]['relative_power'][ch][band] = np.round(relative_power, self.precisionDigit) 
        ...
        for band1 in self.band_freqs:
            for band2 in self.band_freqs:
                if (not band1 == band2) :
                    ...
                    self.segments[segment_name]['band_ratios'][ch][f'{band1}/{band2}'] = np.round(band_ratio, self.precisionDigit)
...
#------------PAIRWISE-------------------------------------------------------------------------  
    ...
    def compute_amplitude_asymmetry(self, segment_name, ch1, ch2): 
        for band in self.band_freqs:
            ...
            amp_asym = np.round( (power_ch1 - power_ch2) / (power_ch1 + power_ch2), self.precisionDigit )
    ...
    def compute_phase_lag(self, segment_name, ch1, ch2):
        data_ch1 = self.segments[segment_name]['segmentData'][ch1]
        data_ch2 = self.segments[segment_name]['segmentData'][ch2]
        ...
        for band in self.band_freqs:
            ...
            self.segments[segment_name]['phase_lag'][ch1][f'to {ch2}_{band}'] = np.round(mean_phase_lag, self.precisionDigit)
    ...
    def compute_coherence(self, segment_name, ch1, ch2):
        ...
        for band in self.band_freqs:
            ...
            coherence_value = np.round(coherence_value, self.precisionDigit)
